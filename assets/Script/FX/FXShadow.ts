...
export class FXShadow extends AutoComponent {
    ...
    ShowFxShadow() {
        ...
        tween(this.shadow)
            .to(this.time, { opacity: 255 }, { easing: 'quadInOut' })
            .call(() => {
                // resolve();
            })
            .start();
    ...
    HideFXShadow() {
        ...
        Tween.stopAllByTarget(this.shadow);
        tween(this.shadow)
            .to(this.time, { opacity: 0 }, { easing: 'quadInOut' })
            .call(() => {
    ...
    ShowFxGuide(guide: Node) {
        ...
        tween(guide)
            .to(this.time, { position: new Vec3(0, posLocal.y, posLocal.z) })
            .call(() => {
                // resolve();
            })
            .start();
    ...
    HideFxGuide(guide: Node) {
        ...
        tween(guide)
            .to(this.time, { position: new Vec3(-1080, posLocal.y, posLocal.z) })
            .call(() => {
                guide.active = false;
                const blockNode = guide.getChildByName('block');
                if (blockNode) {
